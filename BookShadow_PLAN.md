...
### Phase 19：AI 外延发现 + 「待购」状态 ✅（v0.8.5）

**目标**：基于用户藏书偏好，推荐库外书目，供用户决策是否购入；推荐结果以独立「待购」状态存入数据库，与已有三个阅读状态分开管理。

**背景说明**：用户现有三个状态（已读/在读/想读）均指已购入的书，「待购」专门承载"还没买、但 AI 推荐了"的书单。

**实施要点**

- [x] `discover_books` 命令：构建偏好画像 → LLM 生成 10 本候选 → 两层去重（精确匹配 + Jaro-Winkler 模糊，阈值 0.85）→ 不足 5 本时最多循环 3 轮补充 → 返回 5 本（有几本返回几本），书目仅含书名/作者/推荐理由，不含元数据
- [x] `enrich_book` 命令：按需拉取单本元数据（豆瓣 → Google Books → Open Library），供用户点击「加入待购」时调用（懒加载）
- [x] `norm_title()` 标准化：去副标题（`：`/`——`）、版本标注（括号内含"版"）、书名号、全角转半角、小写；配合 `strsim::jaro_winkler` 消除译名/版本变体（如"人类群星闪耀时"≈"人类的群星闪耀时（增订版）"）
- [x] 豆瓣搜索改用内部 JSON API `www.douban.com/j/search?q={}&cat=1001`，从 `sid: NNNNNN` 提取 subject ID，解决原 SPA 渲染导致 HTML 抓取失效的问题
- [x] Tauri 进度事件：`app.emit("discover_progress", msg)` 实时推送各阶段日志；前端 `listen()` 必须在 `invoke()` 前 `await` 注册，避免丢失早期事件；UI 以 `✓`/`›` 指示器展示历史步骤
- [x] `DiscoverModal`：进度日志列表展示；「全部加入待购」按钮加 `addingAll` 状态防重复点击；ESC 用 capture phase + `stopImmediatePropagation()` 拦截，加载中时不响应
- [x] **待购流转**：书卡/列表行悬浮显示橙色「✓ 已购入」按钮，一键将 `tobuy` → 状态未设，移入书库
