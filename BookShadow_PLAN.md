...
1. 构建偏好画像：从高分藏书提取类别、地域、作者、标签偏好（复用 `recommend_books` 逻辑）
2. LLM 生成候选：输出 15-20 本库外推荐（书名 + 作者），结构化 JSON
3. 元数据补全（级联）：
   - 有 Cookie → 先 scrape 豆瓣搜索页 `book.douban.com/search?q={title}`
   - 回退 → Google Books title+author 搜索 API
   - 再回退 → Open Library 搜索 API
   - 全失败 → 仅保留 LLM 给出的书名/作者，无封面/ISBN
4. 去重：过滤已在库（任意状态）的书
5. 展示面板：显示推荐列表，每本书可一键「加入待购」写入数据库；待购列表在主界面筛选面板中独立显示，可进一步筛选/编辑；买到后手动将状态改为想读/在读/已读

**待讨论**

- 豆瓣搜索 scraping 稳定性（结构变化概率高于 ISBN 页）
- 中文书依赖豆瓣，无 Cookie 时中文推荐元数据质量较差
- LLM 推荐的书名/作者准确性（外文书相对可靠，中文书有幻觉风险）
