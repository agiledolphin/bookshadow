...
- **书籍管理**：新增、编辑、删除书籍，支持封面、简介、评分、地域、类别、语言等字段；支持从本地文件上传封面图片
...
- **筛选**：按阅读状态、星级、地域、类别、语言、年代、标签多维筛选；筛选数字联动更新，避免点击后无结果；类别筛选面板同地域一样分主要/其他两级
- **全文搜索**：书名、作者、简介全文搜索（支持中文）
- **书评**：每本书可写多篇 Markdown 书评，支持编辑/预览切换，支持从 `.md` 文件导入
- **网格 / 列表**：两种书籍浏览视图，滚动到底部自动加载更多，窗口大小和位置自动记忆
- **键盘快捷键**：`/` 聚焦搜索框（已有内容则全选），`e` 进入编辑模式，`←` / `→` 在书籍间导航，`ESC` 退出或清空搜索
